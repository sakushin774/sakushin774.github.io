...
let rec apply_logic op arg1 arg2 = match op, arg1, arg2 with
...
let rec eval_exp env = function
    Var x -> 
      (try Environment.lookup x env with 
        Environment.Not_bound -> raise (Unbound x))
  | ILit i -> IntV i
  | BLit b -> BoolV b
  | BinOp (op, exp1, exp2) -> 
...
let rec eval_decl env = function
  ...
  | RecDecl (id, para, exp) ->
    ...
    dummyenv := newenv; 
   (* let v = eval_exp newenv exp in*) (id, newenv, ProcV (para, exp, dummyenv))
  | NewDecl (id, exp, next) ->
